...
BlockFailed::~BlockFailed() {
  std::for_each(begin(), end(), std::function<void (value_type)>(&BlockFailed::delete_entry));
}

inline BlockFailed::iterator
BlockFailed::max_element() {
  return std::max_element(begin(), end(), std::function<bool (value_type, value_type)>(&BlockFailed::compare_entries));
}

inline BlockFailed::reverse_iterator
BlockFailed::reverse_max_element() {
  return std::max_element(rbegin(), rend(), std::function<bool (value_type, value_type)>(&BlockFailed::compare_entries));
}
