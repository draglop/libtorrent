...
log_update_child_cache(int index) {
  log_child_list::const_iterator first =
    std::find_if(log_children.begin(), log_children.end(),
                 std::bind(std::greater_equal<std::pair<int, int> >(), std::placeholders::_1, std::make_pair(index, 0)));
...
void
log_rebuild_cache() {
  std::for_each(log_groups.begin(), log_groups.end(), std::bind(&log_group::clear_cached_outputs, std::placeholders::_1));

  for (int i = 0; i < LOG_GROUP_MAX_SIZE; i++)
    log_update_child_cache(i);

  // Clear the cache...
  std::for_each(log_cache.begin(), log_cache.end(), std::bind(&log_cache_entry::clear, std::placeholders::_1));
  log_cache.clear();
