...
struct hashstring_ptr_hash : public std::function<size_t (const HashString*)> {
...
struct hashstring_hash : public std::function<size_t (HashString)> {
  size_t operator () (const HashString& n) const {
#if USE_ALIGNED
    size_t result = 0;
    const char *first = n.data() + hashstring_hash_ofs;
...
struct hashstring_ptr_equal : public std::function<bool (const HashString*, const HashString*)> {
