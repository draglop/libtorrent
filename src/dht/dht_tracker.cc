...
DhtTracker::prune(uint32_t maxAge) {
  ...
  m_peers.erase(std::remove_if(m_peers.begin(), m_peers.end(), std::bind(&BencodeAddress::empty, std::placeholders::_1)), m_peers.end());
  m_lastSeen.erase(std::remove_if(m_lastSeen.begin(), m_lastSeen.end(), std::bind(std::less<uint32_t>(), std::placeholders::_1, minSeen)), m_lastSeen.end());
